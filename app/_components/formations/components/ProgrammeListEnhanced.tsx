...
import { useProgrammesFormation } from "../../../(pages)/_lib/hooks/useProgrammesFormation";
import { ProgrammeFormation } from "../../_lib/types/programme";
import { useToast } from "../../../(pages)/_lib/hooks/use-toast";
import { generateFormationPDF } from "../../../(pages)/formations/utils/pdfGenerator";
...
export default function ProgrammeListEnhanced({
    ...
    programmes.forEach((programme) => {
      if (programme.categorieId && !categoryIds.has(programme.categorieId)) {
        ...
        categoriesArray.push({
          ...
          titre: programme.categorie?.titre || programme.categorie?.nom || "Catégorie sans titre"
        });
      }
    });
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      // Nous corrigeons l'appel à duplicateProgramme qui ne prend qu'un seul paramètre
      // Le hook s'occupera lui-même de créer une copie
  ...
  const handleGeneratePDF = (programme: ProgrammeFormation) => {
    try {
      ...
      const formationAdapter = {
        ...
        duree: typeof programme.duree === 'number' ? String(programme.duree) : programme.duree,
        ...
        tarif: programme.prix?.toString() || '0', // Mapping de prix à tarif avec conversion en string
            ...
            <Card key={programme.id} className={`overflow-hidden border-l-4 hover:shadow-md transition-shadow ${programme.type === 'catalogue' ? 'border-l-blue-500' : 'border-l-amber-500'}`}>
              ...
              <CardContent className="pb-2">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {programme.categorie && (
                    <Badge variant="outline">{programme.categorie.titre || programme.categorie.nom}</Badge>
                  )}
              ...
              <TableBody>
                {filteredProgrammes.map((programme) => (
                  <TableRow key={programme.id}>
                    ...
                    <TableCell>
                      {programme.categorie ? (programme.categorie.titre || programme.categorie.nom) : "Aucune"}
                    </TableCell>
                    <TableCell>
                      {programme.estActif ? (
